...
`scripts/revalidate_consensus_evidence.py` is the dry-run harness for the next revalidation phase. Without `--live-fetch`, it reads an explicit evidence fixture with `--evidence-file`, filters counted rows by `--product` and `--version`, and classifies rows as malformed, missing a URL, unsupported, pending a source adapter, or structurally ready for future revalidation. In `--live-fetch` mode, `github_issue` supports OBS GitHub Issue revalidation, and Creative COW direct thread URLs support dry-run creator-forum revalidation. Live fetch mode does not run collectors, write evidence rows, change report counts, change verdicts, change evidence state, or promote anything to live consensus.
...
DaVinci Resolve and Adobe Premiere Pro stale report-bearing records remain blocked for generated-record freshness until real adapters or source-access paths verify all counted rows for a record. The Creative COW direct-thread adapter can verify only those specific source rows; Reddit, Adobe Community, and listing-card rows still need separate source-specific handling. Do not refresh DaVinci or Premiere generated-record freshness fields until every counted row for the target record verifies through a real revalidation path.
