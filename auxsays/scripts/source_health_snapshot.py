...
def status_for(source: dict[str, Any], source_state: dict[str, Any], last_error: str) -> tuple[str, str]:
    ...
    if not enabled:
        ...
        if "needs" in recommended or "adapter" in recommended:
            return "Needs adapter", "Needs source-specific adapter"
    ...
    if checked and fetched == 0 and written == 0:
        ...
        return "Active", "No new records"

    if explicit == "degraded":
        return "Error", "Error"
    if source_state.get("last_success_at"):
        return "Active", "Active"
    return "Unknown", "Pending first check"
...
def main() -> int:
    ...
    for source in sources:
        ...
        enabled = bool(source.get("enabled"))
        # Disabled or manually tracked sources should not keep showing stale
        # errors from an older enabled test run. Their current operational state
        # is not actively failing.
        ...
        if not enabled and status == "Needs adapter" and not health_note:
            health_note = "Source configured but intentionally disabled until a source-specific adapter is available."
