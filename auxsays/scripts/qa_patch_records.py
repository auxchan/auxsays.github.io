#!/usr/bin/env python3
"""Warning-first QA for generated AUXSAYS update records."""
...
def add(bucket: list[dict[str, str]], path: Path, code: str, message: str) -> None:
    bucket.append({"file": str(path.relative_to(ROOT)), "code": code, "message": message})
...
def scan_record(path: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if source_type and source_type not in PATCH_NOTE_SOURCE_TYPES | NON_PATCH_NOTE_SOURCE_TYPES | LEGACY_SOURCE_TYPES:
...
def main() -> int:
    ...
    print(f"QA scanned {len(files)} generated records: {len(errors)} errors, {len(warnings)} warnings")
