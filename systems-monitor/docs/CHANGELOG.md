...
- Corrected Release Acceptance lifecycle language, made `publicationClass` the sole fixture/factual discriminant, and clarified spreadsheet formula protection for untrusted text without coercing typed numerics.
- Promoted all seven externally reviewed Foundation contracts to first-binding version `1.0.0`, recording Taylor approval and canonical content hashes while preserving DRAFT history.
- Closed R-001 after approval evidence and advanced authority to Phase-2 contract/design drafting only; UI implementation remains unauthorized until applicable Phase-2 contracts are approved.
- Documented the canonical contract-hash convention in `CONTRACT_TEMPLATE.md` because the existing header required a content hash without defining self-reference handling.
- No dependency, application, Jekyll, workflow, deployment, or Patch Feed change was made.

## 2026-08-17 — Final Foundation audit correction

- Separated accepted and proposed authority in `REPO_FACTS.md`; D-007 and D-008 remain `PROPOSED`.
- Replaced remaining references to the former combined package/build open decision with O-001A through O-001D under their existing authority classifications.
- Added post-V4.1 Taylor review requirements D-009 and D-010 for a four-hour maximum MVP system-evaluation heartbeat, source/release-aware freshness, compute-once/read-many, and bounded infrastructure/API cost governance pending future Master consolidation.
- Amended the DRAFT Infrastructure, Public Data Interface, Repository Integration, and Release Acceptance contracts to version `0.1.1`; no contract was promoted.
- `MASTER_SPEC.md`, application code, Jekyll, workflows, deployment, and Patch Feed were not modified.

## 2026-08-17 — Foundation governance correction

- Corrected D-007 from `ACCEPTED` to `PROPOSED` without changing its decision text.
- Replaced the former combined package/build open decision with O-001A through O-001D, recording engineering authority for package-location and package-manager choices and Taylor approval authority for build-output and Pages-workflow integration.
- No contract status changed; no application, dependency, workflow, deployment, Master, or guardrail file was modified.

## 2026-08-17 — Foundation package 0.1.0

- Installed the unmodified authoritative V4.1 Master Specification repo-locally.
- Created compact guardrails, Master/contract indexes, repository facts, decisions, risks, roadmap, and the lean contract template.
- Drafted Product, Repository Integration, Architecture, Infrastructure, Public Data Interface, Security Ingestion, and Release Acceptance contracts.
- Recorded D-001 through D-008 and Phase-2 open decisions O-001A through O-001D and O-002.
- Added a focused Phase-2 dependency/license candidate review.
- No contract was promoted, no application code was added, no existing AUXSAYS/Patch Feed behavior was changed, and nothing was deployed.
