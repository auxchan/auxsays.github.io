...
export function SystemHealthSummary({ snapshot, variant, setVariant }: { snapshot: PublicSnapshot; variant: FixtureVariant; setVariant: (variant: FixtureVariant) => void }) {
  ...
  return <section className="sm-health" aria-labelledby="sm-health-title"><div><span className="sm-eyebrow">System heartbeat</span><h2 id="sm-health-title">State current <span>Fixture evaluated</span></h2></div><details className="sm-health-details" open><summary>System context details</summary><dl><div><dt>Evaluated</dt><dd>{snapshot.snapshot.evaluatedAt}</dd></div><div><dt>Sources current</dt><dd>{sources.filter((source) => source.freshness === "current").length} / {sources.length}</dd></div><div><dt>New observations</dt><dd>1 synthetic record</dd></div><div><dt>Material changes</dt><dd>1 fixture change</dd></div><div><dt>Next expected release</dt><dd>{sources[0].nextExpectedReleaseAt}</dd></div></dl><div className="sm-source-pulse">{sources.map((source) => <span key={source.sourceId}><FreshnessLabel state={source.freshness} /> {source.dataset}</span>)}</div></details><label className="sm-variant-control">Fixture state lab<select value={variant} onChange={(event) => setVariant(event.target.value as FixtureVariant)}>{snapshot.extensions["auxsays.phase2.fixtureVariants"].map((item) => <option value={item} key={item}>{item}</option>)}</select></label></section>;
}
