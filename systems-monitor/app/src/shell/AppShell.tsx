...
function collectNodes(snapshot: SnapshotViewModel): SearchResult[] {
  ...
  function walk(node: NavigationNode, system: string, path: string[]) {
    results.push({ id: node.id, label: node.label, type: path.length ? "indicator/factor" : "system", context: [system, ...path].join(" / "), state: factual ? "Official reading available" : "OBS / CALC available", freshness: factual ? "local factual candidate" : "current fixture", view: "summary", system, path });
    node.children?.forEach((child) => walk(child, system, [...path, child.slug]));
  }
  snapshot.systems.forEach((system) => walk(system, system.slug, []));
  Object.values(snapshot.sources).forEach((source) => results.push({ id: source.sourceId, label: source.provider, type: "source", context: source.dataset, state: factual ? "Official source evidence" : "OBS evidence", freshness: source.freshness, view: "verified" }));
