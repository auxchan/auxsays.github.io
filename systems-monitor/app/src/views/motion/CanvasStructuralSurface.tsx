...
import type { MotionQaPath, MotionQaRelationship, StructuralSurfaceModel } from "../../data/motionQaReadModel";
...
import { layoutStructuralContextFactors, structuralContextFactors, type StructuralContextFactor } from "./structuralContextFactors";
...
interface CanvasStructuralSurfaceProps {
  ...
  contextFactors?: StructuralContextFactor[];
  surfaceAriaLabel?: string;
  contextLayerAriaLabel?: string;
  nodeLayerAriaLabel?: string;
  relationshipDescription?: string;
  nodeSecondaryLabels?: Map<string, string>;
...
export function CanvasStructuralSurface({ model, path, currentEdges, completedEdgeIds, pathEdgeIds, nodeStates, selectedNodeId, selectedContextFactorId, focusDepth, viewport, traceMode, reducedMotion, reconciliationTargetId, contextFactors = structuralContextFactors, surfaceAriaLabel = "Synthetic structural pressure surface with spatial neighborhoods and continuous routed dependencies", contextLayerAriaLabel = "Synthetic underlying factor previews", nodeLayerAriaLabel = "Synthetic structural nodes", relationshipDescription = "The canvas is supplemented by keyboard-accessible node controls and a complete structured relationship list.", nodeSecondaryLabels, nodeDescriptions, onSelectNode, onSelectContextFactor, onReset }: CanvasStructuralSurfaceProps) {
  ...
  const contextFactorLayouts = useMemo(() => layoutStructuralContextFactors(spatialNodes, camera, structuralDepths, viewport.visibleNodeIds, contextFactors), [spatialNodes, camera, structuralDepths, viewport.visibleNodeIds, contextFactors]);
  ...
  return <div ref={hostRef} className={`sm-viz-surface ${panning || wheelActive ? "is-manipulating" : ""} ${panning ? "is-panning" : ""}`} data-structural-renderer="canvas-rd" data-layout-mode="employment-concentric-orbit" data-orbit-geometry={model.centerNodeId === "fixture-employment" ? "eight-around-one" : `${Math.max(0, spatialNodes.length - 1)}-around-one`} data-relationship-semantics={model.relationships.every((edge) => edge.relationshipClass === "HIERARCHY") ? "hierarchy-navigation-only" : "structural-fixture"} data-fullscreen={fullscreenActive} data-depth-field={reducedMotion ? "static" : "spring-parallax"} data-depth-particle-count={spatialNodes.length * STRUCTURAL_PARTICLES_PER_NODE} data-camera-motion="stable-map-swing-focus" data-context-factor-count={contextFactorLayouts.length} data-selected-context-factor-id={selectedContextFactorId ?? ""} data-trace-mode={traceMode} data-focus-depth={focusDepth} data-visible-relationship-count={viewport.visibleRelationshipIds.size} data-visible-relationship-ids={[...viewport.visibleRelationshipIds].join(" ")} data-hovered-node-id={hoveredNodeId ?? ""} data-connector-motion={reducedMotion ? "static" : traceMode ? "trace" : "ambient"} data-viewport-zoom={viewportTransform.zoom.toFixed(3)} data-viewport-pan-x={Math.round(viewportTransform.panX)} data-viewport-pan-y={Math.round(viewportTransform.panY)} onDoubleClick={handleDoubleClick} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerLeave={() => { if (!panSession.current) parallaxTarget.current = { x: 0, y: 0 }; }} onPointerUp={endPan} onPointerCancel={endPan} onMouseDown={(event) => { if (event.button === 1) event.preventDefault(); }} onAuxClick={(event) => { if (event.button === 1) event.preventDefault(); }}>
    ...
    <div className="sm-viz-context-layer" aria-label={contextLayerAriaLabel}>{contextFactorLayouts.map((factor) => {
      const parent = nodes.get(factor.parentNodeId);
    ...
    <div className="sm-viz-node-layer" aria-label={nodeLayerAriaLabel}>{spatialNodes.map((node) => {
      ...
      const depthVisual = resolveStructuralDepthVisual(structuralDepths.get(node.id) ?? 0, hovered || selectedNodeId === node.id || active || (!selectedNodeId && node.id === model.centerNodeId));
      ...
      }}><i className="sm-viz-node-anchor" aria-hidden="true" data-selected-node-anchor={selectedNodeId === node.id ? "visible" : undefined}><StructuralNodeIcon symbol={visual.symbol} /></i><span><b>{label?.text ?? node.label}</b><small>{nodeSecondaryLabels?.get(node.id) ?? (focusDepth > 0 ? node.kind.replaceAll("_", " ") : "")}</small></span></button>;
    })}</div>
    ...
    })}{currentEdges.some((edge) => edge.outcome === "PARTIALLY_ABSORBED") && <span data-motion-component="absorbed" />}{currentEdges.some((edge) => edge.outcome === "AMPLIFIED") && <span className="sm-motion-amplified-halo" />}{traceMode && commonOriginNodeId && path && <span className="sm-motion-origin-token" data-origin-id={path.originId} data-anchor-node-id={commonOriginNodeId} />}{traceMode && reconciliationTargetId && <span data-common-origin-reconciliation="single" data-anchor-node-id={reconciliationTargetId} />}</div>
  </div>;
}
